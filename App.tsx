...
const ThemeSwitcher: React.FC<{ theme: string; setTheme: (theme: string) => void }> = ({ theme, setTheme }) => (
    <button
        ...
        className="p-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors text-neutral-900 dark:text-neutral-200 hover:text-neutral-900 dark:hover:text-white"
...
const App: React.FC = () => {
  ...
  // Update user last seen on login and periodically
  useEffect(() => {
      if (!currentUser?.id) return;

      // Update immediately on login/load
  ...
  useEffect(() => {
    const initSystem = async () => {
        ...
        if (!activeApiKey) {
            ...
            if (key) {
                setActiveApiKey(key);
                sessionStorage.setItem('esaie_session_api_key', key);
            }
        }
        ...
        // 4. All users now use shared Anti-Captcha API key (no database fetch needed)
    };
        ...
        <main className="flex-1 flex flex-col min-w-0 md:pl-24 lg:pl-28 transition-all duration-300 relative z-10 h-full">
            ...
            <header className="sticky top-0 z-30 w-full border-b-[0.5px] border-neutral-200/80 dark:border-white/5 bg-white/80 dark:bg-[#050505]/80 backdrop-blur-xl shrink-0">
                <div className="w-full px-4 py-3 md:px-6 flex items-center justify-between">
                    ...
                    <div className="md:hidden font-black text-xl tracking-tighter flex items-center gap-2 text-neutral-900 dark:text-white">
                        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-brand-start to-brand-end flex items-center justify-center text-white font-bold text-lg">E</div>
                        <span>ESAIE</span>
                    </div>
                    
                    {/* Desktop spacer */}
                    <div className="hidden md:block">
                        <span className="text-xs font-mono text-neutral-500 dark:text-neutral-500 uppercase tracking-widest">ESAIE.Anti-Captcha.v1</span>
                    </div>
                    ...
                    <div className="flex items-center gap-3 ml-auto">
                        ...
                        <button
                            ...
                            className="hidden md:flex p-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-900 dark:text-neutral-200 hover:text-neutral-900 dark:hover:text-white transition-colors"
                            title="Refresh App"
                        >
                        ...
                        <button
                            ...
                            className="p-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-900 dark:text-neutral-200 hover:text-neutral-900 dark:hover:text-white transition-colors"
                        >
                        ...
                        <button
                            ...
                            className="md:hidden p-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-900 dark:text-neutral-200 hover:text-neutral-900 dark:hover:text-white transition-colors"
                        >
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden p-2 sm:p-3 md:p-6 lg:p-8 custom-scrollbar">
                ...
                {activeView === 'home' && announcements.length > 0 && (
                     <div className="mb-6 mx-auto max-w-[1600px] w-full bg-brand-start/10 dark:bg-brand-start/10 border border-brand-start/20 text-neutral-900 dark:text-white p-2 rounded-xl shadow-[0_0_15px_rgba(249,115,22,0.2)] flex items-center gap-3 animate-zoomIn relative overflow-hidden group">
                        {/* Static Label */}
