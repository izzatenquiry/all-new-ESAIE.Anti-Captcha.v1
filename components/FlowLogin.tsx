...
const FlowLogin: React.FC<FlowLoginProps> = ({ currentUser, onUserUpdate }) => {
    ...
    const [antiCaptchaApiKey, setAntiCaptchaApiKey] = useState(() => {
        // All users use shared Anti-Captcha API key
        return SHARED_ANTICAPTCHA_API_KEY;
    });
    ...
    const [flowEmail, setFlowEmail] = useState<string | null>(null);
    const [flowPassword, setFlowPassword] = useState<string | null>(null);
    const [showFlowPassword, setShowFlowPassword] = useState(false);
    const [copiedFlowEmail, setCopiedFlowEmail] = useState(false);
    const [copiedFlowPassword, setCopiedFlowPassword] = useState(false);
    ...
    useEffect(() => {
        ...
        // All users use shared Anti-Captcha API key (no database fetch needed)
        setAntiCaptchaApiKey(SHARED_ANTICAPTCHA_API_KEY);
        
        // Load flow account email/password if user has email_code
        if (currentUser?.email_code) {
            getEmailPasswordFromCode(currentUser.email_code).then(result => {
                if (result.success) {
                    setFlowEmail(result.account.email);
                    setFlowPassword(result.account.password);
                }
            }).catch(err => {
                console.error('Failed to load flow account:', err);
            });
        } else {
            setFlowEmail(null);
            setFlowPassword(null);
        }
    ...
    }, [currentUser?.personalAuthToken, currentUser?.recaptchaToken, currentUser?.id, currentUser?.email_code]);
    ...
    useEffect(() => {
        ...
        const checkTokenUnchanged = async () => {
            ...
            // All users use shared Anti-Captcha API key
            return antiCaptchaApiKey.trim() === SHARED_ANTICAPTCHA_API_KEY;
        };
        ...
        <div className="w-full">
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                <div className="bg-white dark:bg-neutral-900 rounded-lg shadow-sm p-6 h-full overflow-y-auto">
                    ...
                    <div className="mb-6">
                        <div className="flex items-start gap-2 sm:gap-3 p-3 sm:p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg border-[0.5px] border-orange-200 dark:border-orange-800">
                            <InformationCircleIcon className="w-4 h-4 sm:w-5 sm:h-5 text-orange-600 dark:text-orange-400 flex-shrink-0 mt-0.5" />
                            <div className="text-[11px] sm:text-xs text-orange-800 dark:text-orange-200">
                ...
                <div className="space-y-4">
                    ...
                    {/* Flow Account Credentials - Show if user has email_code */}
                    {currentUser?.email_code && flowEmail && flowPassword && (
                        <div className="mb-4 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-4">
                            <h4 className="text-sm font-semibold text-orange-900 dark:text-orange-200 mb-3">
                                Flow Account Credentials ({currentUser.email_code})
                            </h4>
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <div className="bg-white dark:bg-neutral-800 px-3 py-2 rounded border border-orange-200 dark:border-orange-700 flex items-center justify-between">
                                        ...
                                        <button
                                            ...
                                            onClick={async () => {
                                                try {
                                                    await navigator.clipboard.writeText(flowEmail);
                                                    setCopiedFlowEmail(true);
                                                    setTimeout(() => setCopiedFlowEmail(false), 2000);
                                                } catch (err) {
                                                    console.error('Failed to copy email:', err);
                                                }
                                            }}
                                            className="ml-2 p-1.5 hover:bg-orange-100 dark:hover:bg-orange-800 rounded transition-colors"
                                        ...
                                        >
                                            {copiedFlowEmail ? (
                                                ...
                                                <ClipboardIcon className="w-4 h-4 text-orange-600 dark:text-orange-400" />
                                            )}
                                        </button>
                                    </div>
                                </div>
                                        ...
                                        <div className="bg-white dark:bg-neutral-800 px-3 py-2 rounded border border-orange-200 dark:border-orange-700 flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-1 ml-2">
                                                <button
                                                    ...
                                                    onClick={async () => {
                                                        try {
                                                            await navigator.clipboard.writeText(flowPassword);
                                                            setCopiedFlowPassword(true);
                                                            setTimeout(() => setCopiedFlowPassword(false), 2000);
                                                        } catch (err) {
                                                            console.error('Failed to copy password:', err);
                                                        }
                                                    }}
                                                    className="p-1.5 hover:bg-orange-100 dark:hover:bg-orange-800 rounded transition-colors"
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => setShowFlowPassword(!showFlowPassword)}
                                                    className="p-1.5 hover:bg-orange-100 dark:hover:bg-orange-800 rounded transition-colors"
                                                    aria-label={showFlowPassword ? 'Hide password' : 'Show password'}
                                                    title={showFlowPassword ? 'Hide password' : 'Show password'}
                                                >
                                                    {showFlowPassword ? (
                                                        <EyeOffIcon className="w-4 h-4 text-orange-600 dark:text-orange-400" />
                                                    ) : (
                                ...
                                <div className="bg-orange-100 dark:bg-orange-900/30 border border-orange-300 dark:border-orange-700 rounded p-2 mt-2">
                                    <p className="text-xs text-orange-800 dark:text-orange-200">
                                        <strong>Note:</strong> Use these credentials to login to Google Flow.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <button
                            ...
                            className="w-full flex items-center justify-center gap-2 bg-orange-600 dark:bg-orange-700 text-white text-sm font-semibold py-2.5 px-4 rounded-lg hover:bg-orange-700 dark:hover:bg-orange-600 transition-colors disabled:opacity-50"
                        >
                ...
                <div className="bg-white dark:bg-neutral-900 p-6 rounded-lg shadow-sm h-full overflow-y-auto">
                    ...
                    <div className="mt-8 pt-8 border-t border-neutral-200 dark:border-neutral-700">
                        ...
                        {/* Shared Key Info */}
                        <div className="p-3 sm:p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg border-[0.5px] border-orange-200 dark:border-orange-800 mb-4">
                            <div className="flex items-start gap-2 sm:gap-3">
                                <InformationCircleIcon className="w-4 h-4 sm:w-5 sm:h-5 text-orange-600 dark:text-orange-400 flex-shrink-0 mt-0.5" />
