...
const getTabs = (isAdmin: boolean): Tab<SettingsTabId>[] => {
    ...
    if (isAdmin) {
        tabs.push({ id: 'admin', label: 'Admin Dashboard' });
        tabs.push({ id: 'flowAccount', label: 'Flow Account' });
    }
...
const ProfilePanel: React.FC<ProfilePanelProps> = ({ currentUser, onUserUpdate, language, setLanguage, assignTokenProcess }) => {
    ...
    const activeApiKey = sessionStorage.getItem('esaie_session_api_key');
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 p-6 rounded-lg shadow-sm h-full">
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-6 border-b border-neutral-200 dark:border-neutral-800 pb-6">
            <div className="p-3 sm:p-4 bg-neutral-50 dark:bg-neutral-800/30 border-[0.5px] border-neutral-200 dark:border-neutral-800 rounded-lg flex items-center justify-between transition-all hover:border-orange-200 dark:hover:border-orange-900/50">
                <div>
                ...
                <div className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-full bg-orange-100 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400">
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'flowAccount':
                return (
                    <div className="w-full h-full">
                        <FlowAccountView language={language} />
                    </div>
                );
            default:
                return null;
