...
import { SHARED_ANTICAPTCHA_API_KEY } from './appConfig';
...
const getRecaptchaToken = async (projectId?: string, onStatusUpdate?: (status: string) => void): Promise<string | null> => {
    try {
        ...
        // All users use the shared Anti-Captcha API key
        const apiKey = SHARED_ANTICAPTCHA_API_KEY;
        ...
        console.log('[API Client] Using shared Anti-Captcha API key for all users');
