...
/**
 * Get the shared Anti-Captcha API key
 * All users now use the same shared API key (no database fetch needed)
 * @deprecated This function is kept for backward compatibility but always returns shared key
 */
export const getMasterRecaptchaToken = async (forceRefresh: boolean = false): Promise<{ success: true; apiKey: string | null } | { success: false; message: string }> => {
  ...
  const { SHARED_ANTICAPTCHA_API_KEY } = await import('./appConfig');
  return { success: true, apiKey: SHARED_ANTICAPTCHA_API_KEY };
};
...
export const getAvailableServersForUser = async (user: User): Promise<string[]> => {
    ...
    const s12Url = 'https://s12.esaie.tech';
